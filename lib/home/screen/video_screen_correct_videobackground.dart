...
//   try {
//     // handle BetterPlayerEvent if available
//     if (event.betterPlayerEventType ==
//         BetterPlayerEventType.exception) {
//       // Pause when an exception occurs to avoid background playback
//       try {
...
//     // ✅ When video starts playing, remove loader
...
//           qualities
//               .add(HlsQuality(label: label, url: absoluteUrl, bitrate: bitrate));
//         }
//       }
//     }
...
//       if (mounted && !_isDragging && !_isLocked) {
...
//   setState(() {
//     _isSeeking = true; // ✅ Show loader immediately
//   });
...
//   if (_isLocked) return;
//   _isInFullscreen = true;  // ✅ Set flag before pushing
...
//     ),
//   );
//   _isInFullscreen = false;  // ✅ Clear flag when returned
//   if (mounted) {
...
//       }
//     } catch (e) {
//       debugPrint('Error pausing controller on deactivate: $e');
//     }
//   }
//   super.deactivate();
// }
...
//     // ✅ FIX: WillPopScope intercepts Android back button & swipe back
//     return WillPopScope(
//       onWillPop: () async {
...
//                                 brightness =
//                                     (brightness - delta).clamp(0.0, 1.0).toDouble();
...
//                                     details.primaryDelta! / 300;
//                                 volume =
//                                     (volume - delta).clamp(0.0, 1.0).toDouble();
...
//                         child: Center(
//                             child:
//                                 _sideIndicator(Icons.volume_up, volume)),
//                       ),
//                   ],
//                 ),
//               ),
...
//   @override
//   State<HotstarFullscreenPage> createState() =>
//       _HotstarFullscreenPageState();
// }
...
//                   _brightness =
//                       (_brightness - delta).clamp(0.0, 1.0).toDouble();
//                   await ScreenBrightness()
...
//                   brightnessTimer =
//                       Timer(const Duration(milliseconds: 800), () {
//                     if (mounted) setState(() => showBrightnessUI = false);
//                   });
//                 },
//                 child: const SizedBox.expand(),
//               ),
...
//                   volumeTimer =
//                       Timer(const Duration(milliseconds: 800), () {
//                     if (mounted) setState(() => showVolumeUI = false);
//                   });
//                 },
//                 child: const SizedBox.expand(),
//               ),
...
//   @override
//   State<HotstarSettingsDialog> createState() =>
//       _HotstarSettingsDialogState();
// }
...
//   // ✅ Guard all async callbacks after dispose
...
//   // ✅ Loading state — show loader until video is initialized
...
//     // ✅ Stop all callbacks if already disposed
//     if (_isDisposed) return;
...
//     // ✅ Mark video as ready when initialized so the loader disappears
...
//   // ✅ Properly stops background playback before popping
//   Future<bool> _handleBackPress() async {
//     // Mark disposed FIRST so no callbacks fire during cleanup
//     _isDisposed = true;
...
//     // Small delay to ensure pause takes effect before dispose
//     await Future.delayed(const Duration(milliseconds: 100));
...
//         final bitrate = bitrateMatch != null ? int.parse(bitrateMatch.group(1)!) : 0;
//         final resolution = resolutionMatch != null ? resolutionMatch.group(1)! : "";
...
//               ? _resolutionToLabel(resolution)
//               : "${(bitrate / 1000).round()} kbps";
//           qualities.add(HlsQuality(label: label, url: absoluteUrl, bitrate: bitrate));
//         }
//       }
//     }
...
//   // ✅ For portrait videos, the surface is still attached — just seek and play
//   if (isPortraitVideo) {
//     await Future.delayed(const Duration(milliseconds: 300));
//     if (!mounted || _isDisposed) return;
//     try {
...
//     setState(() => _showControls = true);
//     _startHideTimer();
//     return; // ✅ Skip re-setup for portrait
//   }

//   // Landscape: needs re-attach after orientation change
...
//                 final imageUrl = item['image']?.toString() ?? '';
//                 final videoTrailer = item['videoTrailer']?.toString() ?? '';
//                 final videoMovies = item['videoMovies']?.toString() ?? videoTrailer;
//                 final subtitle = item['subtitle']?.toString() ?? '';
//                 final dis = item['dis']?.toString() ?? '';
...
//   @override
//   void deactivate() {
//     // ✅ Pause on deactivate but only if not going to fullscreen
//     if (!_isInFullscreen && !_isDisposed) {
//       try {
//         _controller.pause();
//       } catch (e) {
...
//                 child: Stack(
//                   children: [
//                     // ✅ Always render BetterPlayer (black bg while loading)
//                     BetterPlayer(controller: _controller),

//                     // ✅ Full-screen loader shown until video is initialized
//                     if (!_isVideoReady)
//                       Positioned.fill(
//                         child: Container(
//                           color: Colors.black,
...
//                     // ✅ Top bar always visible so user can go back even during load
//                     if (_showControls && !_isLocked)
//                       Positioned(
//                         top: 0,
//                         left: 0,
...
//   void initState() {
//     super.initState();
//     // ✅ Start from the position passed by the parent
...
//   // ✅ Use addPostFrameCallback so pop happens after current frame
...
//                       child: Center(
//                         child: AspectRatio(
//                           aspectRatio: widget.controller.videoPlayerController
//                                   ?.value.aspectRatio ??
//                               (16 / 9),
//                           child: BetterPlayer(controller: widget.controller),
//                         ),
