...
Color _getCurrentColor() {
 ...
 Future<void> _extractColors() async {
  ...
  for (final item in sourcelist) {
    try {
      ...
      if (item is BannerMovie) {
        imageUrl = item.mobileImage.isNotEmpty ? item.mobileImage : item.logoImage;
  ...
  Future<Color> _dominantColorFromNetwork(String url) async {
    ...
    return Color.fromARGB(255, (rSum / count).round(), (gSum / count).round(),
        (bSum / count).round());
  }
              ...
              slivers: [
                ...
                SliverPersistentHeader(
                  ...
                  delegate: CategorySelectorDelegate(
                      child: _categorySelector(), height: 70.h),
                ),
                SliverPadding(
                ...
                children: [
                  logoUrl.isNotEmpty
                      ? Image.network(logoUrl,
                          height: 50.h, width: 180.w, fit: BoxFit.contain)
                      : Text(titleText,
                          style: TextStyle(
...
// }
  ...
  controller.selectCategory(index);
  setState(() {
    _bannerTopColors = []; // purane colors clear karo
    _currentIndex = 0;     // index reset karo
  });
  // Naye data load hone ke baad colors extract karo
  ever(controller.bannerMovies, (_) => _extractColors());
}
  ...
  // void _navigateToBannerDetail(BannerMovie banner) {
...
void _navigateToBannerDetail(BannerMovie banner) async {
  if (banner.isSingleMovie && banner.effectiveMovieId != null) {
    ...
    // ✅ Fetch movie detail, trailer, and play URL using the MOVIE id
    final results = await Future.wait([
      BannerMovieService.fetchMovieDetail(movieId),       // Map<String, dynamic>?
      BannerMovieService.fetchMovieTrailerUrl(movieId),   // String
      BannerMovieService.fetchMoviePlayUrl(movieId),      // String?
    ]);
...
String _extractUrl(dynamic field) {
...
// ── CategorySelectorDelegate (unchanged) ──────────────────────────────────────
